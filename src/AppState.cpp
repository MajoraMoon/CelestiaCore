...
void StateManager::setupSubscriptions() {
  ...
  eventBus.subscribe<ToggleMouseVisibilityEvent>(
      [this](const ToggleMouseVisibilityEvent &) {
        state.windowMouseVisible = !state.windowMouseVisible;
        eventBus.publish(MouseVisibilityChanged{state.windowMouseVisible});
      });

  // GUI visibility
  eventBus.subscribe<ToggleGuiVisibilityEvent>(
      [this](const ToggleGuiVisibilityEvent &) {
        state.guiManagerGuiVisible = !state.guiManagerGuiVisible;
        eventBus.publish(GuiVisibilityChanged{state.guiManagerGuiVisible});
      });

  // Pause state
  eventBus.subscribe<TogglePauseEvent>([this](const TogglePauseEvent &) {
    state.frameTimerSimulationPaused = !state.frameTimerSimulationPaused;
    eventBus.publish(SimulationPausedChanged{state.frameTimerSimulationPaused});
  });
  ...
  eventBus.subscribe<QuitEvent>([this](const QuitEvent &) {
    state.celestiaCoreQuit = true;
    eventBus.publish(CelestiaCoreQuitChanged{state.celestiaCoreQuit});
  });
  ...
  eventBus.subscribe<SetMouseSensitivityEvent>(
      [this](const SetMouseSensitivityEvent &ev) {
        ...
        if (state.cameraMouseSensitivity != ev.sensitivity) {
          state.cameraMouseSensitivity = ev.sensitivity;
          eventBus.publish(MouseSensitivityChanged{ev.sensitivity});
        }
      });

  // Fullscreen mode
  eventBus.subscribe<SetFullscreenModeEvent>(
      [this](const SetFullscreenModeEvent &ev) {
...
void StateManager::publishInitialStates() {
  ...
  eventBus.publish(SimulationPausedChanged{state.frameTimerSimulationPaused});
  eventBus.publish(WindowMaximizedChanged{state.windowMaximized});
  eventBus.publish(MouseSensitivityChanged{state.cameraMouseSensitivity});
