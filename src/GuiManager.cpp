...
void GuiManager::showStatsWindow()
{
    ...
    if (ImGui::SliderFloat("Mouse Sensitivity", &m_mouseSensitivity, 0.01f, 1.0f, "%.2f"))
    {
    ...
    int currentScreenMode = static_cast<int>(window.getFullscreenMode());
    if (ImGui::Combo("Display Mode", &currentScreenMode, screenModes, IM_ARRAYSIZE(screenModes)))
    {
        window.setFullscreenMode(static_cast<WindowSDLGL::FullscreenMode>(currentScreenMode));
    }
